...
sh_ver="1.4.0"
github="raw.githubusercontent.com/shakebbq-dot/tools/refs/heads/main/"
...
simulate_installbbr(){
    echo "安装BBR内核: ${release} ${version} ${bit}"
    if [[ "${release}" == "centos" ]]; then
        echo "执行: 导入elrepo GPG与安装 kernel-ml-4.11.8 及 headers/devel"
    else
        echo "执行: 下载并安装 linux-headers/image 4.11.8 deb 包"
    fi
    echo "执行: detele_kernel"
    echo "执行: BBR_grub"
    echo "提示: 重启后重新运行脚本开启BBR/BBR魔改版"
}
simulate_installbbrplus(){
    echo "安装BBRplus内核: ${release} ${version} ${bit}"
    if [[ "${release}" == "centos" ]]; then
        echo "执行: 下载并安装 kernel-4.14.129-bbrplus.rpm"
    else
        echo "执行: 下载并安装 linux-headers/image 4.14.129-bbrplus deb 包"
    fi
    echo "执行: detele_kernel"
    echo "执行: BBR_grub"
    echo "提示: 重启后重新运行脚本开启BBRplus"
}
simulate_installlot(){
    echo "安装Lotserver内核: ${release} ${version} ${bit}"
    if [[ "${release}" == "centos" ]]; then
        echo "执行: 安装 firmware/kernel/headers/devel 指定版本"
    else
        echo "执行: 通过远程脚本安装兼容内核"
    fi
    echo "执行: detele_kernel"
    echo "执行: BBR_grub"
    echo "提示: 重启后重新运行脚本开启Lotserver"
}
simulate_startbbr(){
    echo "使用BBR加速"
    echo "执行: remove_all"
    echo "执行: 写入 net.core.default_qdisc 与 net.ipv4.tcp_congestion_control=bbr 并 sysctl -p"
}
...
simulate_startbbrmod(){
    ...
    echo "执行: 写入 tsunami 并 sysctl -p"
}
simulate_startbbrmod_nanqinlang(){
    echo "使用暴力BBR魔改版加速"
    echo "执行: 安装工具链并编译加载 tcp_nanqinlang.ko"
    echo "执行: 写入 nanqinlang 并 sysctl -p"
}
simulate_startlotserver(){
    echo "使用Lotserver加速"
    echo "执行: 安装 ethtool 与远程安装 LotServer"
    echo "执行: 修改 /appex/etc/config 并重启 LotServer"
}
simulate_remove_all(){
    echo "卸载全部加速"
    echo "执行: 删除 /etc/sysctl.conf 中相关键与卸载 LotServer"
}
simulate_optimizing_system(){
    echo "系统配置优化"
    echo "执行: 清理旧键并追加网络与文件句柄参数、sysctl -p、写 limits.conf 与 profile"
    echo "提示: 重启后生效"
}
simulate_Update_Shell(){
    echo "更新脚本"
    echo "执行: 下载最新 tcp.sh 并 chmod +x"
}
simulate_detele_kernel(){
    echo "删除多余内核"
    echo "执行: 列出非当前版本并循环卸载"
}
simulate_BBR_grub(){
    echo "更新引导"
    if [[ "${release}" == "centos" ]]; then
        echo "执行: CentOS6 修改 grub.conf default=0 或 CentOS7 grub2-set-default 0"
    else
        echo "执行: update-grub"
    fi
}
simulate_check_status(){
    echo "检查状态"
    echo "执行: 解析 uname 与 sysctl、lsmod 判定状态"
}
dry_run_main(){
    echo "开始干跑测试: ${release} ${version} ${bit}"
    simulate_installbbr
    simulate_installbbrplus
    simulate_installlot
    simulate_startbbr
